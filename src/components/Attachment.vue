<template>
  <div>
    <div class="container mt-10">
      ...
      >
        <div class="dropzone-container flex justify0center items-center">
      ...
      <div class="my-10">Note you can upload maximum 10 resume</div>
      <div class="h-6 w-full bg-gray-200 rounded-lg">
        <div
          class="h-6 bg-blue-600 rounded-lg flex items-center justify-center text-white"
          :style="{ width: this.width * 10 + '%' }"
        >{{ this.width * 10 }}%</div>
      </div>
      <div class="flex justify-end">{{ cv }}</div>
...
export default {
  ...
  methods: {
    ...
    fileAdded(file) {
      ...
      console.log("myyy atttcment", this.tempAttachments);
    },
  ...
  computed: {
    cv() {
      if (this.tempAttachments.length <= 10) {
        return 10 - this.tempAttachments.length + " Cvs left";
      } else {
        return "you cant add more";
      }
    },
    width() {
      if (this.tempAttachments.length <= 10) {
